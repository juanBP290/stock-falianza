...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'delete_proforma' && target === 'srv') {
      return jsonOut_(
        deleteProformaRows_(ss, payload.proforma_num, payload.row_numbers || payload.sheet_rows || [])
      );
    }

    if (action === 'delete_rows' && target === 'srv') {
      return jsonOut_(deleteRowsByNumbers_(ss, SHEET_SERVICIOS, payload.row_numbers || []));
    }
...
function deleteProformaRows_(ss, proformaNum, rowNumbers) {
  const sh = ss.getSheetByName(SHEET_SERVICIOS);
  if (!sh) {
    return { ok: false, error: 'No existe la hoja ' + SHEET_SERVICIOS };
  }
  ...
  const target = String(proformaNum || '').trim().toUpperCase();
  if (!target && (!Array.isArray(rowNumbers) || !rowNumbers.length)) {
    return { ok: false, error: 'Falta proforma_num o row_numbers' };
  }

  let deletedByNumber = 0;
  if (Array.isArray(rowNumbers) && rowNumbers.length) {
    deletedByNumber = deleteRowsByNumbers_(ss, SHEET_SERVICIOS, rowNumbers).deleted || 0;
  }

  if (!target) {
    return { ok: true, deleted: deletedByNumber };
  }

  return {
    ok: true,
    deleted: deletedByNumber + deleteProformaRowsFromSheet_(sh, target, headers, idxProforma),
  };
}

function deleteRowsByNumbers_(ss, sheetName, rowNumbers) {
  ...
  if (!Array.isArray(rowNumbers) || !rowNumbers.length) {
    return { ok: false, error: 'Falta row_numbers' };
  }

  const uniqueRows = Array.from(
    new Set(
      rowNumbers.map(function (x) {
